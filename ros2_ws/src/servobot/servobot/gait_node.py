...
class GaitNode(Node):
    ...
    def gait_pos(self, leg: int, t: float, vel: tuple[float, float], ang: float, origin: tuple[float, float, float]) -> tuple[float, float, float]:
        ...
        angle = (self.turn_angles[leg] if abs(ang) > 0.1 else math.atan2(vel[1], vel[0]))
        ...
        z = self.swing_height * h * abs(amplitude)
        ...
        x = l * self.swing_length * amplitude * math.cos(angle)
        y = l * self.swing_length * amplitude * math.sin(angle)
